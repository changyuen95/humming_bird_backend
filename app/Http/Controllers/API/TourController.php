    ...
    public function getToursAjax(Request $request)
{
    ...
    // Filter by season (using the `season` column in the `tours` table)
    if ($filter_params['season'] && !in_array('All', $filter_params['season'])) {
        $query->whereIn('season', $filter_params['season']);
    }

    // Filter by destinations (using the `region` column in the `tours` table)
    if ($filter_params['destinations'] && !in_array('All', $filter_params['destinations'])) {
        $query->whereIn('region', $filter_params['destinations']);
    }
